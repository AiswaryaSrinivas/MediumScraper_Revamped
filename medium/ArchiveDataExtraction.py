...
Modified on 29 October 2018

# http://jsoneditoronline.org/ to View JSON

We will only extract a few columns that are required for analysis and discard the rest

The following columns are considered for Posts :

['id','scrappedDate','createdAt','firstPublishedAt','latestPublishedAt','updatedAt','imageCount','linksCount',
...
#import sqlalchemy
#import urllib
#os.chdir("/Users/aiswarya/DataScienceArena/medium_scraper/")
...
def processPost(dat):
    ...
    posts_dict["createdAt"]=[str(dat["createdAt"])]
    posts_dict["updatedAt"]=[str(dat["updatedAt"])]
    posts_dict["firstPublishedAt"]=[str(dat["firstPublishedAt"])]
    posts_dict["latestPublishedAt"]=[str(dat["latestPublishedAt"])]
    ...
    posts_dict["importedPublishedAt"]=[str(dat["importedPublishedAt"])]
    posts_dict["vote"]=[dat["vote"]]
    ...
    posts_dict["seriesLastAppendedAt"]=[str(dat["seriesLastAppendedAt"])]
    ...
    posts_dict["featureLockRequestAcceptedAt"]=[str(dat["featureLockRequestAcceptedAt"])]
    if "featureLockRequestMinimumGuaranteeAmount" in dat:
        posts_dict["featureLockRequestMinimumGuaranteeAmount"]=[dat["featureLockRequestMinimumGuaranteeAmount"]]
    else:
        posts_dict["featureLockRequestMinimumGuaranteeAmount"]=[0]
    ...
    if "virtuals" in dat:
        ...
        for tag in virtuals["tags"]:
            ...
            if 'metadata' in tag and 'followerCount' in tag['metadata']:
                ...
                tags_dict["followerCount"]=[0]

            tags_dict["postCount"]=[tag['metadata']["postCount"]]
            if 'virtuals' in tag and 'isFollowing' in tag['virtuals']:
                tags_dict["isFollowing"]=[tag['virtuals']['isFollowing']]
            else:
                tags_dict["isFollowing"]=[0]
...
def ReadData(path):
    ...
    for fileName in files :
        if '.json' in fileName:
            ...
            scrappedDate_split=scrappedDate.split("//")
            scrappedDate=scrappedDate_split[len(scrappedDate_split)-1]
